...
class GameState(State):
    ...
    def calculate_gold_reward(self, playerInfo, stage=0):
        if stage == 0:
            ...
            if playerInfo._bonus_left <= 0:
                ...
                del playerInfo._bonus_left
                return total

            playerInfo._reward_total += 1
            playerInfo._bonus_left -= 1
            return self.calculate_gold_reward(playerInfo, stage=1)
    ...
    def playHand(self):
        ...
        if "Micheal Myers" in owned:
            hand_mult+=random.randrange(0,23)
    ...
    def discardCards(self, removeFromHand: bool, cards_discarded = 1):
        self.hand.remove(self.cardsSelectedList.pop())
        if len(self.cardsSelectedList) != 0:
            self.discardCards(True, cards_discarded+1)
        else:
            self.hand.extend(State.deckManager.dealCards(self.deck, cards_discarded))
        self.updateCards(400, 520, self.cards, self.hand, scale=1.2)
